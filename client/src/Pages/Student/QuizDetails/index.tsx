...
import { Container, Stack, Typography, Button, AccessTimeFilledIcon, Skeleton } from '../../../mui';
import { useSnackBar, useAuth, useModal } from '../../../Hooks';
...
function QuizDetails() {
  ...
  useEffect(() => {
    const validQuery = ['private', 'public'].includes(searchParams.get('type') as string)
      && autoCompleteOptions.includes(publicTitle);
    if (!validQuery && !isPrivateQuiz) {
  ...
  const { teacher_name: teacherName, title, description, time } = quiz;
  ...
  return (
    <Container className={classes.container} sx={{ marginBlock: '3rem' }}>
      {!loading ? (
        <>
          <Typography component="div" variant="h4" color="primary" className={classes.title}>
            {title}
          </Typography>
          ...
          <Stack component="div" flexDirection="row" justifyContent="center" alignItems="center">
            <span className={classes.timeLimit}>Exact Time Limit: </span>
